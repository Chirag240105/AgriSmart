...
import { TrendingUp, TrendingDown, Leaf, ShoppingCart, Package, DollarSign, Activity, AlertCircle, MessageSquare, Cloud } from 'lucide-react';
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const crops = (cropsRes.data?.data || cropsRes.data?.crops || []).map((crop) => ({
                    id: crop._id,
                    name: crop.cropName,
                ...
                const orders = (ordersRes.data?.data || ordersRes.data?.orders || []).map((order) => ({
                    id: order._id,
        ...
        <div className="space-y-6">
            ...
            <Card className="bg-gradient-to-r from-green-500 to-green-600 text-white border-0">
                <CardHeader>
                    ...
                    <CardDescription className="text-green-50">{t('dashboard.manageOps')}</CardDescription>
                </CardHeader>
            </Card>
            ...
            {user?.role === 'farmer' && (
                <>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            {chartData.length === 0 ? (
                                <p className="text-sm text-muted-foreground">{t('dashboard.noRevenueData')}</p>
                            ) : (
                    ...
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <Link to="/crops">
                                <Button variant="outline" className="w-full mt-4">{t('dashboard.viewAllCrops')}</Button>
                            </Link>
                        </CardContent>
                    </Card>
                </>
                ...
                <>
                    ...
                    <Card>
                        ...
                        <CardContent>
                                    ...
                                    <div key={order.id} className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-sm text-muted-foreground">
                                                {order.quantity} {t('crops.quantity')} &bull; {new Date(order.createdAt).toLocaleDateString()}
                                            </p>
                                        </div>
                                        <div className="text-right">
                            ...
                            <Link to="/orders">
                                <Button variant="outline" className="w-full mt-4">{t('dashboard.viewAllOrders')}</Button>
                            </Link>
                        </CardContent>
                    </Card>
                </>
            ...
            <Card>
                ...
                <CardContent>
                    ...
                    <Link to="/prices">
                        <Button variant="outline" className="w-full mt-4">{t('dashboard.latestPrices')}</Button>
                    </Link>
                </CardContent>
            </Card>
