...
const parseLocation = (shipment) => {
    const coordinates = shipment?.currentLocation?.coordinates;
    if (Array.isArray(coordinates)) return `${coordinates[1].toFixed(4)}, ${coordinates[0].toFixed(4)}`;
    if (typeof shipment?.currentLocation === 'string') return shipment.currentLocation;
    return shipment?.currentLocation?.address || shipment?.destination?.address || 'Unknown';
};
...
export const ShipmentsPage = () => {
        ...
        const fetchShipments = async () => {
            try {
                ...
                const mapped = (response.data?.data || []).map((shipment) => ({
                    ...
                    currentLocation: parseLocation(shipment),
    ...
    const renderShipmentCard = (shipment, index) => (
        <motion.div key={shipment.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
            <Card>
                <CardHeader>
