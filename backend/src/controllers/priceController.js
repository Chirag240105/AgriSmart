...
const fetchMlLatestPrices = async () => {
  const defaultCrops = [
...
export const getLatestPrices = async (req, res) => {
  try {
    const { cropName, limit = 20, source } = req.query;

    if (source === "ml") {
      const data = await fetchMlLatestPrices();
      return res.json({ success: true, data });
    }

    // Always use fresh ML predictions for accurate data
    const data = await fetchMlLatestPrices();
    const filtered = cropName
      ? data.filter((d) => d.cropName.toLowerCase().includes(cropName.toLowerCase()))
      : data;
    return res.json({ success: true, data: filtered.slice(0, Number(limit)) });
  } catch (err) {
    console.error("getLatestPrices error:", err?.message);
