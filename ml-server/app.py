...
def load_disease_model():
    ...
    if not TF_AVAILABLE:
        print("[WARNING] TensorFlow not available. Disease model will not load.")
        print(f"[WARNING] TensorFlow import error: {TF_IMPORT_ERROR}")
        return
...
def run_prediction(image_bytes):
    if not TF_AVAILABLE:
        return {
            "success": False,
            "error": "TensorFlow is not available on this runtime.",
            "details": TF_IMPORT_ERROR
        }
